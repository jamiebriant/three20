...
static BFFURLCache*          gSharedCache = nil;
...
- (id)initWithName:(NSString*)name {
  if (self == [super init]) {
    _name             = [name copy];
    _cachePath        = [[BFFURLCache cachePathWithName:name] retain];
    _invalidationAge  = BFF_DEFAULT_CACHE_INVALIDATION_AGE;
...
- (void)dealloc {
  ...
  BFF_RELEASE_SAFELY(_imageCache);
  BFF_RELEASE_SAFELY(_imageSortedList);
  BFF_RELEASE_SAFELY(_cachePath);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (BFFURLCache*)cacheWithName:(NSString*)name {
  if (!gNamedCaches) {
    gNamedCaches = [[NSMutableDictionary alloc] init];
  }
  BFFURLCache* cache = [gNamedCaches objectForKey:name];
  if (!cache) {
    cache = [[[BFFURLCache alloc] initWithName:name] autorelease];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (BFFURLCache*)sharedCache {
  if (!gSharedCache) {
    gSharedCache = [[BFFURLCache alloc] init];
  }
  return gSharedCache;
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
+ (void)setSharedCache:(BFFURLCache*)cache {
...
- (void)expireImagesFromMemory {
  while (_imageSortedList.count) {
    ...
    BFFDCONDITIONLOG(BFFDFLAG_URLCACHE, @"EXPIRING %@", key);
...
- (NSData*)dataForURL:(NSString*)URL {
  return [self dataForURL:URL expires:BFF_CACHE_EXPIRATION_AGE_NEVER timestamp:nil];
}
...
- (void)removeAll:(BOOL)fromDisk {
  ...
  if (fromDisk) {
    ...
    [BFFURLCache createPathIfNecessary:_cachePath];
  }
}
...
- (void)logMemoryUsage {
  ...
  BFFDCONDITIONLOG(BFFDFLAG_URLCACHE, @"======= IMAGE CACHE: %d images, %d pixels ========",
                  _imageCache.count, _totalPixelCount);
  NSEnumerator* e = [_imageCache keyEnumerator];
  for (NSString* key ; key = [e nextObject]; ) {
    UIImage* image = [_imageCache objectForKey:key];
    BFFDCONDITIONLOG(BFFDFLAG_URLCACHE, @"  %f x %f %@", image.size.width, image.size.height, key);
  }
#endif
}
