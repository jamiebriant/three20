...
- (void)updateTabStyles {
  ...
  if (self.tabViews.count > columnCount) {
    int column = 0;
    for (BFFTab* tab in self.tabViews) {
      if (column == 0) {
...
- (CGSize)layoutTabs {
  if (self.width && self.height) {
    BFFGridLayout* layout = [[[BFFGridLayout alloc] init] autorelease];
    layout.padding = 1;
