...
// UI (private)
#import "BFFSearchTextFieldInternal.h"
...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
@implementation BFFSearchTextField
...
- (id)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    _internal = [[BFFSearchTextFieldInternal alloc] initWithTextField:self];
...
- (void)dealloc {
  ...
  BFF_RELEASE_SAFELY(_internal);
  BFF_RELEASE_SAFELY(_tableView);
  BFF_RELEASE_SAFELY(_shadowView);
  BFF_RELEASE_SAFELY(_screenView);
  BFF_RELEASE_SAFELY(_previousNavigationItem);
  BFF_RELEASE_SAFELY(_previousRightBarButtonItem);
...
- (void)showDoneButton:(BOOL)show {
  UIViewController* controller = [BFFNavigator navigator].visibleViewController;
  if (controller) {
    if (show) {
...
- (void)showDarkScreen:(BOOL)show {
  if (show && !_screenView) {
    ...
    _screenView.backgroundColor = BFFSTYLEVAR(screenBackgroundColor);
...
- (UITableView*)tableView {
  if (!_tableView) {
    _tableView = [[BFFTableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
    _tableView.backgroundColor = BFFSTYLEVAR(searchTableBackgroundColor);
    _tableView.separatorColor = BFFSTYLEVAR(searchTableSeparatorColor);
...
- (void)showSearchResults:(BOOL)show {
  if (show && _dataSource) {
    ...
    if (!_shadowView) {
      _shadowView = [[BFFView alloc] init];
      _shadowView.style = BFFSTYLE(searchTableShadow);
...
- (CGRect)rectForSearchResults:(BOOL)withKeyboard {
  ...
  CGFloat keyboardHeight = withKeyboard ? BFFKeyboardHeight() : 0;
