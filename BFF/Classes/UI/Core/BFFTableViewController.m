...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
@implementation BFFTableViewController
...
- (void)dealloc {
  ...
  BFF_RELEASE_SAFELY(_tableDelegate);
  BFF_RELEASE_SAFELY(_dataSource);
  BFF_RELEASE_SAFELY(_tableView);
  BFF_RELEASE_SAFELY(_loadingView);
  BFF_RELEASE_SAFELY(_errorView);
  BFF_RELEASE_SAFELY(_emptyView);
  BFF_RELEASE_SAFELY(_tableOverlayView);
  BFF_RELEASE_SAFELY(_tableBannerView);
...
- (void)createInterstitialModel {
  self.dataSource = [[[BFFTableViewInterstitialDataSource alloc] init] autorelease];
}
...
- (void)viewDidUnload {
  ...
  BFF_RELEASE_SAFELY(_menuView);
  [_menuCell removeFromSuperview];
  BFF_RELEASE_SAFELY(_menuCell);
}
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  } else if ([_tableView isKindOfClass:[BFFTableView class]]) {
    BFFTableView* tableView = (BFFTableView*)_tableView;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
#pragma mark BFFViewController


///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)keyboardDidAppear:(BOOL)animated withBounds:(CGRect)bounds {
  [super keyboardDidAppear:animated withBounds:bounds];
  self.tableView.frame = BFFRectContract(self.tableView.frame, 0, bounds.size.height);
...
- (void)showLoading:(BOOL)show {
  if (show) {
    if (!self.model.isLoaded || ![self canShowModel]) {
      ...
      if (title.length) {
        BFFActivityLabel* label = [[[BFFActivityLabel alloc] initWithStyle:BFFActivityLabelStyleWhiteBox]
                                  autorelease];
        label.text = title;
  ...
  if (show) {
    if (!self.model.isLoaded || ![self canShowModel]) {
      ...
      if (title.length || subtitle.length || image) {
        BFFErrorView* errorView = [[[BFFErrorView alloc] initWithTitle:title
                                                            subtitle:subtitle
      ...
      if ([_dataSource respondsToSelector:@selector(tableView:willUpdateObject:atIndexPath:)]) {
        ...
        if (newIndexPath) {
          if (newIndexPath.length == 1) {
            BFFDCONDITIONLOG(BFFDFLAG_TABLEVIEWMODIFICATIONS, @"UPDATING SECTION AT %@", newIndexPath);
            ...
            BFFDCONDITIONLOG(BFFDFLAG_TABLEVIEWMODIFICATIONS, @"UPDATING ROW AT %@", newIndexPath);
  ...
  if (model == _model) {
    if (_isViewAppearing && _flags.isShowingModel) {
      if ([_dataSource respondsToSelector:@selector(tableView:willInsertObject:atIndexPath:)]) {
        ...
        if (newIndexPath) {
          if (newIndexPath.length == 1) {
            BFFDCONDITIONLOG(BFFDFLAG_TABLEVIEWMODIFICATIONS, @"INSERTING SECTION AT %@", newIndexPath);
            NSInteger sectionIndex = [newIndexPath indexAtPosition:0];
            [_tableView insertSections:[NSIndexSet indexSetWithIndex:sectionIndex]
                      withRowAnimation:UITableViewRowAnimationTop];
          } else if (newIndexPath.length == 2) {
            BFFDCONDITIONLOG(BFFDFLAG_TABLEVIEWMODIFICATIONS, @"INSERTING ROW AT %@", newIndexPath);
  ...
  if (model == _model) {
    if (_isViewAppearing && _flags.isShowingModel) {
      if ([_dataSource respondsToSelector:@selector(tableView:willRemoveObject:atIndexPath:)]) {
        ...
        if (newIndexPath) {
          if (newIndexPath.length == 1) {
            BFFDCONDITIONLOG(BFFDFLAG_TABLEVIEWMODIFICATIONS, @"DELETING SECTION AT %@", newIndexPath);
            NSInteger sectionIndex = [newIndexPath indexAtPosition:0];
            [_tableView deleteSections:[NSIndexSet indexSetWithIndex:sectionIndex]
                      withRowAnimation:UITableViewRowAnimationLeft];
          } else if (newIndexPath.length == 2) {
            BFFDCONDITIONLOG(BFFDFLAG_TABLEVIEWMODIFICATIONS, @"DELETING ROW AT %@", newIndexPath);
...
- (UITableView*)tableView {
  if (nil == _tableView) {
    _tableView = [[BFFTableView alloc] initWithFrame:self.view.bounds style:_tableViewStyle];
    _tableView.autoresizingMask =  UIViewAutoresizingFlexibleWidth
    | UIViewAutoresizingFlexibleHeight;

    UIColor* backgroundColor = _tableViewStyle == UITableViewStyleGrouped
    ? BFFSTYLEVAR(tableGroupedBackgroundColor)
    : BFFSTYLEVAR(tablePlainBackgroundColor);
    if (backgroundColor) {
...
- (void)setTableBannerView:(UIView*)tableBannerView animated:(BOOL)animated {
  BFF_INVALIDATE_TIMER(_bannerTimer);
...
- (id<UITableViewDelegate>)createDelegate {
  if (_variableHeightRows) {
    return [[[BFFTableViewVarHeightDelegate alloc] initWithController:self] autorelease];
  } else {
    return [[[BFFTableViewDelegate alloc] initWithController:self] autorelease];
  }
}
...
- (void)showMenu:(UIView*)view forCell:(UITableViewCell*)cell animated:(BOOL)animated {
  ...
  if (animated) {
    [UIView beginAnimations:nil context:nil];
    [UIView setAnimationDuration:BFF_FAST_TRANSITION_DURATION];
