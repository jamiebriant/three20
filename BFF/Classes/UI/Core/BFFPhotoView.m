...
- (id)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    _photoVersion = BFFPhotoVersionNone;
...
- (void)dealloc {
  ...
  BFF_RELEASE_SAFELY(_photo);
  BFF_RELEASE_SAFELY(_captionLabel);
  BFF_RELEASE_SAFELY(_captionStyle);
  BFF_RELEASE_SAFELY(_statusSpinner);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (BOOL)loadVersion:(BFFPhotoVersion)version fromNetwork:(BOOL)fromNetwork {
  NSString* URL = [_photo URLForVersion:version];
  if (URL) {
    UIImage* image = [[BFFURLCache sharedCache] imageForURL:URL];
...
- (void)showCaption:(NSString*)caption {
  if (caption) {
    if (!_captionLabel) {
      _captionLabel = [[BFFLabel alloc] init];
      _captionLabel.opaque = NO;
      _captionLabel.style = _captionStyle ? _captionStyle : BFFSTYLE(photoCaption);
...
- (void)setImage:(UIImage*)image {
  if (image != _defaultImage || !_photo || self.urlPath != [_photo URLForVersion:BFFPhotoVersionLarge]) {
...
- (void)imageViewDidFailLoadWithError:(NSError*)error {
  [self showProgress:0];
  if (error) {
    [self showStatus:BFFDescriptionForError(error)];
  }
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)layoutSubviews {
  CGRect screenBounds = BFFScreenBounds();
  CGFloat width = self.width;
  CGFloat height = self.height;
  CGFloat cx = self.bounds.origin.x + width/2;
  CGFloat cy = self.bounds.origin.y + height/2;
  CGFloat marginRight = 0, marginLeft = 0, marginBottom = BFFToolbarHeight();
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)setPhoto:(id<BFFPhoto>)photo {
...
- (BOOL)loadPreview:(BOOL)fromNetwork {
  if (![self loadVersion:BFFPhotoVersionLarge fromNetwork:NO]) {
    if (![self loadVersion:BFFPhotoVersionSmall fromNetwork:NO]) {
      if (![self loadVersion:BFFPhotoVersionThumbnail fromNetwork:fromNetwork]) {
        return NO;
      }
    }
  }
...
- (void)loadImage {
  if (_photo) {
    _photoVersion = BFFPhotoVersionLarge;
    self.urlPath = [_photo URLForVersion:BFFPhotoVersionLarge];
  }
}
...
- (void)showStatus:(NSString*)text {
  if (text) {
    if (!_statusLabel) {
      _statusLabel = [[BFFLabel alloc] init];
      _statusLabel.style = BFFSTYLE(photoStatusLabel);
