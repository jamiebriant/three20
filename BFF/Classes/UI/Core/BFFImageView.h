...
@class BFFURLRequest;
@protocol BFFImageViewDelegate;

/**
 * A view that asynchronously loads an image and subsequently displays it.
 */
@interface BFFImageView : BFFView <BFFURLRequestDelegate> {
  ...
  id<BFFImageViewDelegate> _delegate;
}
 ...
 */
@property (nonatomic, assign) id<BFFImageViewDelegate> delegate;
