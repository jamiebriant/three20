...
// UI
#import "BFFTextView.h"
#import "BFFTextEditorInternal.h"
...
// before or after the BFFTextEditor expands or contracts its height to match.  Obviously,
...
- (id)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    _internal = [[BFFTextEditorInternal alloc] initWithTextEditor:self];
...
- (void)createTextView {
  if (!_textView) {
    _textView = [[BFFTextView alloc] init];
...
- (void)constrainToText {
  ...
  if (/* JAB oldHeight && */ diff) { // JAB: Need to be able to recalculate height on demand, not just the first time.
    ...
    self.frame = BFFRectContract(self.frame, 0, -diff);
  }
}
...
- (void)layoutSubviews {
  ...
  _textView.frame = CGRectOffset(BFFRectContract(frame, 0, 14), 0, 7);
  _textField.frame = CGRectOffset(BFFRectContract(frame, 9, 14), 9, 7);
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)setDelegate:(id<BFFTextEditorDelegate>)delegate {
  _delegate = delegate;
