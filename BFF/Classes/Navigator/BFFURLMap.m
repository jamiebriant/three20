...
// UINavigator (private)
#import "BFFUIViewController+BFFNavigatorGarbageCollection.h"
...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
@implementation BFFURLMap


///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)dealloc {
  BFF_RELEASE_SAFELY(_objectMappings);
  BFF_RELEASE_SAFELY(_objectPatterns);
  BFF_RELEASE_SAFELY(_fragmentPatterns);
  BFF_RELEASE_SAFELY(_stringPatterns);
  BFF_RELEASE_SAFELY(_schemes);
  BFF_RELEASE_SAFELY(_defaultObjectPattern);
  [super dealloc];
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)addObjectPattern: (BFFURLNavigatorPattern*)pattern
                  forURL: (NSString*)URL {
  pattern.URL = URL;
  [pattern compile];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)addStringPattern: (BFFURLGeneratorPattern*)pattern
                  forURL: (NSString*)URL
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (BFFURLNavigatorPattern*)matchObjectPattern:(NSURL*)URL {
  if (_invalidPatterns) {
    [_objectPatterns sortUsingSelector:@selector(compareSpecificity:)];
    _invalidPatterns = NO;
  }

  for (BFFURLNavigatorPattern* pattern in _objectPatterns) {
...
- (void)from:(NSString*)URL toObject:(id)target {
  BFFURLNavigatorPattern* pattern = [[BFFURLNavigatorPattern alloc] initWithTarget:target];
...
- (void)from:(Class)cls toURL:(NSString*)URL {
  BFFURLGeneratorPattern* pattern = [[BFFURLGeneratorPattern alloc] initWithTargetClass:cls];
...
- (void)setObject:(id)object forURL:(NSString*)URL {
  if (nil == _objectMappings) {
    _objectMappings = BFFCreateNonRetainingDictionary();
  }
...
- (id)objectForURL: (NSString*)URL
           ...
           pattern: (BFFURLNavigatorPattern**)outPattern {
  id object = nil;
  if (_objectMappings) {
  ...
  BFFURLNavigatorPattern* pattern  = [self matchObjectPattern:theURL];
  if (pattern) {
    if (!object) {
      object = [pattern createObjectFromURL:theURL query:query];
    }
    if (pattern.navigationMode == BFFNavigationModeShare && object) {
...
- (id)dispatchURL:(NSString*)URL toTarget:(id)target query:(NSDictionary*)query {
  NSURL* theURL = [NSURL URLWithString:URL];
  for (BFFURLNavigatorPattern* pattern in _fragmentPatterns) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (BFFNavigationMode)navigationModeForURL:(NSString*)URL {
  NSURL* theURL = [NSURL URLWithString:URL];
  ...
  return BFFNavigationModeExternal;
}


///////////////////////////////////////////////////////////////////////////////////////////////////
- (NSInteger)transitionForURL:(NSString*)URL {
  BFFURLNavigatorPattern* pattern = [self matchObjectPattern:[NSURL URLWithString:URL]];
...
- (NSString*)URLForObject:(id)object withName:(NSString*)name {
  ...
  while (cls) {
    ...
    BFFURLGeneratorPattern* pattern = [_stringPatterns objectForKey:key];
    if (pattern) {
