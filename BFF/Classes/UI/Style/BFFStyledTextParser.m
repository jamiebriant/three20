...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
@implementation BFFStyledTextParser
...
- (void)dealloc {
  ...
  BFF_RELEASE_SAFELY(_chars);
  BFF_RELEASE_SAFELY(_stack);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)addNode:(BFFStyledNode*)node {
  if (!_rootNode) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)pushNode:(BFFStyledElement*)element {
  if (!_stack) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)popNode {
  BFFStyledElement* element = [_stack lastObject];
  if (element) {
...
- (void)parseURLs:(NSString*)string {
  ...
  while (stringIndex < string.length) {
    ...
    if (startRange.location == NSNotFound) {
      ...
      BFFStyledTextNode* node = [[[BFFStyledTextNode alloc] initWithText:text] autorelease];
      [self addNode:node];
      break;
      ...
      if (endRange.location == NSNotFound) {
        ...
        BFFStyledLinkNode* node = [[[BFFStyledLinkNode alloc] initWithText:URL] autorelease];
        node.URL = URL;
        [self addNode:node];
        break;
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName
  ...
  if ([tag isEqualToString:@"span"]) {
    BFFStyledInline* node = [[[BFFStyledInline alloc] init] autorelease];
    node.className =  [attributeDict objectForKey:@"class"];
    [self pushNode:node];

  } else if ([tag isEqualToString:@"br"]) {
    BFFStyledLineBreakNode* node = [[[BFFStyledLineBreakNode alloc] init] autorelease];
    node.className =  [attributeDict objectForKey:@"class"];
    [self pushNode:node];

  } else if ([tag isEqualToString:@"div"] || [tag isEqualToString:@"p"]) {
    BFFStyledBlock* node = [[[BFFStyledBlock alloc] init] autorelease];
    node.className =  [attributeDict objectForKey:@"class"];
    [self pushNode:node];

  } else if ([tag isEqualToString:@"b"]) {
    BFFStyledBoldNode* node = [[[BFFStyledBoldNode alloc] init] autorelease];
    [self pushNode:node];

  } else if ([tag isEqualToString:@"i"]) {
    BFFStyledItalicNode* node = [[[BFFStyledItalicNode alloc] init] autorelease];
    [self pushNode:node];

  } else if ([tag isEqualToString:@"a"]) {
    BFFStyledLinkNode* node = [[[BFFStyledLinkNode alloc] init] autorelease];
    node.URL =  [attributeDict objectForKey:@"href"];
    [self pushNode:node];

  } else if ([tag isEqualToString:@"button"]) {
    BFFStyledButtonNode* node = [[[BFFStyledButtonNode alloc] init] autorelease];
    node.URL =  [attributeDict objectForKey:@"href"];
    [self pushNode:node];

  } else if ([tag isEqualToString:@"img"]) {
    BFFStyledImageNode* node = [[[BFFStyledImageNode alloc] init] autorelease];
    node.className =  [attributeDict objectForKey:@"class"];
  ...
  if (_parseLineBreaks) {
    ...
    while (1) {
      ...
      if (range.location != NSNotFound) {
        ...
        BFFStyledLineBreakNode* br = [[[BFFStyledLineBreakNode alloc] init] autorelease];
        [self addNode:br];
    ...
    BFFStyledTextNode* node = [[[BFFStyledTextNode alloc] initWithText:string] autorelease];
    [self addNode:node];
  }
}
