...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)dealloc {
  BFF_RELEASE_SAFELY(_highlightedLabel);
...
- (void)touchesBegan:(NSSet*)touches withEvent:(UIEvent*)event {
  ...
  if ([self.delegate respondsToSelector:@selector(tableView:touchesBegan:withEvent:)]) {
    id<BFFTableViewDelegate> delegate = (id<BFFTableViewDelegate>)self.delegate;
...
- (void)touchesEnded:(NSSet*)touches withEvent:(UIEvent*)event {
  ...
  if (_highlightedLabel) {
    BFFStyledElement* element = _highlightedLabel.highlightedNode;
    ...
    if ([element isKindOfClass:[BFFStyledLinkNode class]]) {
      BFFOpenURL([(BFFStyledLinkNode*)element URL]);

    } else if ([element isKindOfClass:[BFFStyledButtonNode class]]) {
      BFFOpenURL([(BFFStyledButtonNode*)element URL]);
...
- (void)layoutSubviews {
  ...
  if (0 == [indexPathsForVisibleRows count]) {
    ...
    BFF_RELEASE_SAFELY(_topShadow);

    [_bottomShadow removeFromSuperlayer];
    BFF_RELEASE_SAFELY(_bottomShadow);
    return;
  }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)setHighlightedLabel:(BFFStyledTextLabel*)label {
