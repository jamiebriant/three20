...
@protocol BFFStyledTextDelegate;
@class BFFStyledNode;
@class BFFStyledFrame;
@class BFFStyledBoxFrame;

@interface BFFStyledText : NSObject <BFFURLRequestDelegate> {
  BFFStyledNode*   _rootNode;
  BFFStyledFrame*  _rootFrame;
  UIFont*         _font;
  CGFloat         _width;
  CGFloat         _height;
  ...
  id<BFFStyledTextDelegate> _delegate;
}

@property (nonatomic, assign) id<BFFStyledTextDelegate> delegate;

/**
 * The first in the sequence of nodes that contain the styled text.
 */
@property (nonatomic, retain) BFFStyledNode* rootNode;

/**
 * The first in the sequence of frames of text calculated by the layout.
 */
@property (nonatomic, readonly) BFFStyledFrame* rootFrame;
...
- (void)insertChild:(BFFStyledNode*)child atIndex:(NSInteger)index;
