...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
@implementation BFFMessageController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
  if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]) {
    _fields = [[NSArray alloc] initWithObjects:
               [[[BFFMessageRecipientField alloc] initWithTitle: BFFLocalizedString(@"To:", @"")
                                                      required: YES] autorelease],
               [[[BFFMessageSubjectField alloc] initWithTitle: BFFLocalizedString(@"Subject:", @"")
                                                    required: NO] autorelease],
               nil];

    self.title = BFFLocalizedString(@"New Message", @"");

    self.navigationItem.leftBarButtonItem = [[[UIBarButtonItem alloc]
                                              initWithTitle: BFFLocalizedString(@"Cancel", @"")
                                              style: UIBarButtonItemStyleBordered
                                              target: self
                                              action: @selector(cancel)] autorelease];
    self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc]
                                               initWithTitle: BFFLocalizedString(@"Send", @"")
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)dealloc {
  BFF_RELEASE_SAFELY(_dataSource);
  BFF_RELEASE_SAFELY(_fields);
  BFF_RELEASE_SAFELY(_initialRecipients);
...
- (void)createFieldViews {
  ...
  for (BFFMessageField* field in _fields) {
    BFFPickerTextField* textField = [field createViewForController:self];
    if (textField) {
      textField.delegate = self;
      textField.backgroundColor = BFFSTYLEVAR(backgroundColor);
      textField.font = BFFSTYLEVAR(messageFont);
      ...
      label.font = BFFSTYLEVAR(messageFont);
      label.textColor = BFFSTYLEVAR(messageFieldTextColor);
      [label sizeToFit];
      ...
      separator.backgroundColor = BFFSTYLEVAR(messageFieldSeparatorColor);
...
- (BOOL)hasEnteredText {
  for (int i = 0; i < _fields.count; ++i) {
    BFFMessageField* field = [_fields objectAtIndex:i];
    if (field.required) {
      if ([field isKindOfClass:[BFFMessageRecipientField class]]) {
        BFFPickerTextField* textField = [_fieldViews objectAtIndex:i];
        if (textField.cells.count) {
          return YES;
        }

      } else if ([field isKindOfClass:[BFFMessageTextField class]]) {
...
- (UITextField*)subjectField {
  for (int i = 0; i < _fields.count; ++i) {
    BFFMessageField* field = [_fields objectAtIndex:i];
    if ([field isKindOfClass:[BFFMessageSubjectField class]]) {
...
- (void)loadView {
  ...
  _scrollView = [[[UIScrollView class] alloc] initWithFrame:BFFKeyboardNavigationFrame()];
  _scrollView.backgroundColor = BFFSTYLEVAR(backgroundColor);
  ...
  _textEditor = [[BFFTextEditor alloc] initWithFrame:CGRectMake(0, 0, _scrollView.width, 0)];
  _textEditor.delegate = self;
  _textEditor.backgroundColor = BFFSTYLEVAR(backgroundColor);
  _textEditor.font = BFFSTYLEVAR(messageFont);
...
- (void)viewDidUnload {
  ...
  BFF_RELEASE_SAFELY(_fieldViews);
  BFF_RELEASE_SAFELY(_textEditor);
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation {
  ...
  _scrollView.height = self.view.height - BFFKeyboardHeight();
  [self layoutViews];
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
#pragma mark UTViewController (BFFCategory)
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)textField:(BFFPickerTextField*)textField didAddCellAtIndex:(NSInteger)cellIndex {
  [self updateSendCommand];
}


///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)textField:(BFFPickerTextField*)textField didRemoveCellAtIndex:(NSInteger)cellIndex {
  [self updateSendCommand];
}


///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)textFieldDidResize:(BFFPickerTextField*)textField {
  [self layoutViews];
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
#pragma mark BFFTextEditorDelegate


///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)textEditorDidChange:(BFFTextEditor*)textEditor {
  [self updateSendCommand];
  _isModified = YES;
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (BOOL)textEditor:(BFFTextEditor*)textEditor shouldResizeBy:(CGFloat)height {
  _textEditor.frame = BFFRectContract(_textEditor.frame, 0, -height);
  [self layoutViews];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)setDataSource:(id<BFFTableViewDataSource>)dataSource {
  if (dataSource != _dataSource) {
    ...
    for (UITextField* textField in _fieldViews) {
      if ([textField isKindOfClass:[BFFPickerTextField class]]) {
        BFFPickerTextField* menuTextField = (BFFPickerTextField*)textField;
...
- (void)addRecipient:(id)recipient forFieldAtIndex:(NSUInteger)fieldIndex {
  self.view;
  BFFPickerTextField* textField = [_fieldViews objectAtIndex:fieldIndex];
  if ([textField isKindOfClass:[BFFPickerTextField class]]) {
...
- (BOOL)fieldHasValueAtIndex:(NSUInteger)fieldIndex {
  ...
  if (fieldIndex == _fieldViews.count) {
    ...
    BFFMessageField* field = [_fields objectAtIndex:fieldIndex];
    if ([field isKindOfClass:[BFFMessageRecipientField class]]) {
      BFFPickerTextField* pickerTextField = [_fieldViews objectAtIndex:fieldIndex];
...
- (void)send {
  ...
  for (int i = 0; i < fields.count; ++i) {
    ...
    if ([field isKindOfClass:[BFFMessageRecipientField class]]) {
      BFFPickerTextField* textField = [_fieldViews objectAtIndex:i];
      [(BFFMessageRecipientField*)field setRecipients:textField.cells];
    } else if ([field isKindOfClass:[BFFMessageTextField class]]) {
      UITextField* textField = [_fieldViews objectAtIndex:i];
      [(BFFMessageTextField*)field setText:textField.text];
    }
  }

  BFFMessageTextField* bodyField = [[[BFFMessageTextField alloc] initWithTitle:nil
                                                               required:NO] autorelease];
...
- (void)confirmCancellation {
  UIAlertView* cancelAlertView = [[[UIAlertView alloc] initWithTitle:
    ...
    delegate:self
    cancelButtonTitle:BFFLocalizedString(@"Yes", @"")
...
- (void)showActivityView:(BOOL)show {
  ...
  if (show) {
    if (!_activityView) {
      ...
      _activityView = [[BFFActivityLabel alloc] initWithFrame:frame
                                               style:BFFActivityLabelStyleWhiteBox];
...
- (NSString*)titleForSending {
  return BFFLocalizedString(@"Sending...", @"");
}
