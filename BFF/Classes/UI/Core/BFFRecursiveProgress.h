...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
@protocol BFFRecursiveProgressDelegate
@required
/**
 ...
 * Coding example:
 *
 * BFFRecursiveProgress* progress = [BFFRecursiveProgress progressWithDelegate:self];
 * progress.percent = 0.5; // didSetProgress:0.5
 *
 * BFFRecursiveProgress* subProgress = [BFFRecursiveProgress
 *   progressWithParent: progress firstPercent: 0.2 lastPercent: 0.6];
 * subProgress.percent = 0.5; // didSetProgress:0.4
 */
@interface BFFRecursiveProgress : NSObject {
  id<BFFRecursiveProgressDelegate>  _delegate;  // Only valid in the topmost node.
  BFFRecursiveProgress*              _parent;    // Only valid in child nodes.
 ...
 */
@property (nonatomic, assign) id<BFFRecursiveProgressDelegate> delegate;


+ (id)progressWithDelegate:(id<BFFRecursiveProgressDelegate>)delegate;

+ (id)progressWithParent: (BFFRecursiveProgress*)parent
            firstPercent: (CGFloat)firstPercent
             lastPercent: (CGFloat)lastPercent;

// Designated initializer for root nodes
- (id)initWithDelegate:(id <BFFRecursiveProgressDelegate>)delegate;

// Designated initializer for child nodes
- (id)initWithParent:(BFFRecursiveProgress*)parent
        firstPercent: (CGFloat)firstPercent
         lastPercent: (CGFloat)lastPercent;
