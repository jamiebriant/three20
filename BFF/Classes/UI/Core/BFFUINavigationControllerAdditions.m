 ...
 * Additions.
 */
@implementation UINavigationController (BFFCategory)
...
- (void)addSubcontroller:(UIViewController*)controller animated:(BOOL)animated
  ...
  if (animated && transition) {
    if ([self isKindOfClass:[BFFNavigationController class]]) {
      [(BFFNavigationController*)self pushViewController: controller
...
- (void)persistNavigationPath:(NSMutableArray*)path {
  for (UIViewController* controller in self.viewControllers) {
    [[BFFNavigator navigator] persistController:controller path:path];
  }
}
